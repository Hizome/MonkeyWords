...
const App = () => {
  ...
  const loadWords = async (lang: string, lvl: number = 1) => {
    try {
      ...
      setWordPool(data);
      const pageWords = generatePageWords(data, 18);
      setWords(pageWords);
  ...
  const nextPage = async () => {
    ...
    const pageWords = generatePageWords(wordPool, 17);
    setWords(pageWords);
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-8 select-none" onClick={() => {
